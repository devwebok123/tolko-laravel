...
class Form
{
    ...
    /**
     * The model alias form translations.
     *
     * @var string
     */
    public $modelAlias;
     ...
     * @return self
     */
    public function nullableRadio(string $name, array $options = [], array $params = []) : self
    {
        if (empty($options)) {
            $options = [
                0 => __('global.no'),
        ...
        $params['options'] = $options;
        $this->resolveFieldConfig($name, $params);
        $this->addField('nullableRadio', $params);
     ...
     * @return self
     */
    public function button(string $type, array $options) : self
    {
        $this->addField('button', $options);
     ...
     * @return self
     */
    public function phone(string $name, array $options = []) : self
    {
        $options['addClass'] = 'mask-phone';
    ...
    {
        $key = 'cruds.' . $this->getModelKey() . '.fields.';

        preg_match("/^([a-z_]+)?\[([a-z_]+)?]/i", $name, $matches);
     ...
     * @return array
     */
    public function addField(string $tag, array $options = []) : array
    {
        $options['tag']     = $tag;
     ...
     * @return array
     */
    public function addFieldRangeFromTo(string $tag, array $options = []) : array
    {
    ...
    protected function getModelKey() : string
    {
        if (!empty($this->modelAlias)) {
            return Str::camel($this->modelAlias);
     ...
     * @return void
     */
    public function addCard(string $name, array $params)
    {
        $this->cards[$name] = $params;
    }
     ...
     * @return self
     */
    public function rawHtml(string $name, array $params) : self
    {
        $params['name'] = $name;
        $this->addField('rawHtml', $params);
        return $this;
    }
}
